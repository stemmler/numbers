...
class App < Sinatra::Base
  ...
  # to help safe guard since this is in memory right now
  # we want to make sure we don't overflow. this is very low, we
  # could go higher, but it's good for now.
  MaxNumberOfUsers = 1_000
  ...
  post '/v1/register' do
    ...
    if @@users.length >= MaxNumberOfUsers
      status 400
      error_message = "Max number of users is #{ MaxNumberOfUsers }. "\
  ...
  # catchalls so that we return a proper JSON response
  # with the 404 (otherwise, sinatra returns html)

  get '/*' do
    status 404
    return JSON.dump( { error: 'Unknown route' } )
  end
